...
- **Acceptance criteria:**
  - `git init` performed with `main` as the default branch; an empty initial commit on `main` carries `README.md`, `PLAN.md`, `CLAUDE.md`. — *Why:* A real initial commit gives every later branch a common ancestor; without it, `git log` and PR diffs are messy.
  - `develop` branched from `main` and pushed; `develop` becomes the **default branch** on GitHub. — *Why:* CLAUDE.md says PRs target `develop`; making it the default branch prevents PRs accidentally landing on `main` (which we'll reserve for tagged releases later).
